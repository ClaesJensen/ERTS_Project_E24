...
#pragma HLS ALLOCATION instances = mul limit = 2 operation
  ...
  // Allpass value (single coefficient for all 5 allpass filters)
  static const coeff_t a_value_allpass = 0.85f;

  // Comb values (feedback coefficients)
  // Could also be placed in a static array, but small so it's typically fine as
  // is.
  static const coeff_t a_values_comb[9] = {0.65f, 0.75f, 0.67f, 0.75f, 0.85f,
                                           0.66f, 0.85f, 0.75f, 0.81f};
  ...
  */
  ...
  // ---------------------------------------------------------------
  *outputData = ((ap_fixed<16, 4>)1.0f - mix) * inputData + mix * allpassOut;
}
