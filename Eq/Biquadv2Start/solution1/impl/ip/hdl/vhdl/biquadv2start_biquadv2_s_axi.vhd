...
entity biquadv2start_biquadv2_s_axi is
generic (
...
end entity biquadv2start_biquadv2_s_axi;
...
--        others   - reserved
-- 0x34 : reserved
-- 0x38 : Data signal of inDataLeft_V
--        bit 23~0 - inDataLeft_V[23:0] (Read/Write)
--        others   - reserved
-- 0x3c : reserved
-- 0x40 : Data signal of inDataRight_V
--        bit 23~0 - inDataRight_V[23:0] (Read/Write)
--        others   - reserved
-- 0x44 : reserved
-- (SC = Self Clear, COR = Clear on Read, TOW = Toggle on Write, COH = Clear on Handshake)

architecture behave of biquadv2start_biquadv2_s_axi is
    ...
    constant ADDR_INDATALEFT_V_DATA_0  : INTEGER := 16#38#;
    constant ADDR_INDATALEFT_V_CTRL    : INTEGER := 16#3c#;
    constant ADDR_INDATARIGHT_V_DATA_0 : INTEGER := 16#40#;
    constant ADDR_INDATARIGHT_V_CTRL   : INTEGER := 16#44#;
    ...
    signal int_inDataLeft_V    : UNSIGNED(23 downto 0) := (others => '0');
    signal int_inDataRight_V   : UNSIGNED(23 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_INDATALEFT_V_DATA_0 =>
                        rdata_data <= RESIZE(int_inDataLeft_V(23 downto 0), 32);
                    when ADDR_INDATARIGHT_V_DATA_0 =>
                        rdata_data <= RESIZE(int_inDataRight_V(23 downto 0), 32);
                    when others =>
                        rdata_data <= (others => '0');
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    inDataLeft_V         <= STD_LOGIC_VECTOR(int_inDataLeft_V);
    inDataRight_V        <= STD_LOGIC_VECTOR(int_inDataRight_V);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_INDATALEFT_V_DATA_0) then
                    int_inDataLeft_V(23 downto 0) <= (UNSIGNED(WDATA(23 downto 0)) and wmask(23 downto 0)) or ((not wmask(23 downto 0)) and int_inDataLeft_V(23 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_INDATARIGHT_V_DATA_0) then
                    int_inDataRight_V(23 downto 0) <= (UNSIGNED(WDATA(23 downto 0)) and wmask(23 downto 0)) or ((not wmask(23 downto 0)) and int_inDataRight_V(23 downto 0));
                end if;
            end if;
        end if;
    end process;
